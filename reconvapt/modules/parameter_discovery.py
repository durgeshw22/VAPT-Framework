#!/usr/bin/env python3
"""
RECON VAPT Framework - Parameter Discovery Module
2 tools: arjun, ffuf
"""
...
class ParameterDiscovery:
    def __init__(self):
        self.tools = {
            "1": {
                ...
                "command": "arjun -u {url} -o {output}",
                "install": "pip3 install arjun"
            },
            "2": {
                "name": "ffuf",
                "description": "Fast parameter fuzzer",
                "command": "ffuf -u '{url}?FUZZ=test' -w /usr/share/wordlists/wfuzz/others/common_pass.txt -mc 200 -o {output} -of csv -s",
    ...
    def show_menu(self):
        print(f"\n{Fore.YELLOW}╔══════════════════════════════════════════════════════════╗")
        print(f"{Fore.YELLOW}║         PARAMETER DISCOVERY - Tool Selection             ║")
    ...
    def run_tool(self, tool_info, url, output_file):
        ...
        try:
            ...
            if process.returncode == 0:
                print(f"{Fore.GREEN}[✓] Done in {elapsed}s{Style.RESET_ALL}")
                print(f"{Fore.GREEN}[✓] Output: {output_file}{Style.RESET_ALL}")
                return True
            return False
        except:
            return False
    ...
    def run(self):
        while True:
            ...
            output_dir = create_output_dir(f"params_{domain}")
