#!/usr/bin/env python3
"""
RECON VAPT Framework - JavaScript Discovery Module
2 tools: linkfinder, httpx
"""
...
class JavaScriptDiscovery:
    def __init__(self):
        self.tools = {
            "1": {
                "name": "curl",
                "description": "Extract JavaScript file references",
                "command": "curl -s '{url}' | grep -oE '(src|href)=\"[^\"]*\\.js[^\"]*\"' | cut -d'\"' -f2 | sort -u > {output} 2>&1 || echo 'No JavaScript files found' > {output}",
                "install": "echo 'curl is pre-installed'"
            },
            "2": {
                "name": "httpx",
                "description": "Detect JavaScript technologies",
                "command": "echo '{url}' | httpx -tech-detect -silent | grep -iE 'javascript|jquery|angular|react|vue|webpack' > {output} 2>&1 || echo 'No JS tech detected' > {output}",
    ...
    def show_menu(self):
        print(f"\n{Fore.YELLOW}╔══════════════════════════════════════════════════════════╗")
        print(f"{Fore.YELLOW}║        JAVASCRIPT DISCOVERY - Tool Selection             ║")
    ...
    def run_tool(self, tool_info, url, output_file):
        ...
        print(f"{Fore.YELLOW}[*] Searching for JavaScript files and endpoints...{Style.RESET_ALL}\n")
        ...
        try:
            ...
            process = subprocess.run(command, shell=True, timeout=600)
            ...
            if os.path.exists(output_file):
                ...
                if file_size > 0:
                    ...
                    with open(output_file, 'r') as f:
                        ...
                        if lines:
                            print(f"\n{Fore.CYAN}[*] Preview (first 10 results):{Style.RESET_ALL}")
                            for line in lines:
                                print(f"{Fore.WHITE}  {line.strip()}{Style.RESET_ALL}")
                            if len(lines) == 10:
                                print(f"{Fore.YELLOW}  ... (more in output file){Style.RESET_ALL}")
                        else:
                            print(f"{Fore.YELLOW}[!] No JavaScript files or endpoints found{Style.RESET_ALL}")
                    return True
                else:
                    print(f"\n{Fore.YELLOW}[!] No results found in {elapsed}s{Style.RESET_ALL}")
                    print(f"{Fore.YELLOW}[*] This URL may not have detectable JavaScript files{Style.RESET_ALL}")
                    return False
            else:
                print(f"\n{Fore.YELLOW}[!] No output generated{Style.RESET_ALL}")
                return False
        except subprocess.TimeoutExpired:
            print(f"\n{Fore.RED}[!] Timeout (10 minutes){Style.RESET_ALL}")
            return False
    ...
    def run(self):
        while True:
            ...
            output_dir = create_output_dir(f"js_{domain}")
